...
export const getReturnsByProduct = async (req, res) => {
  try {
    const { productId } = req.params; // productId will come from URL params
    const userId = req.user._id; // If you want only current user's returns, keep this

    // Find returns matching product
    ...
    if (!returns || returns.length === 0) {
      return res.status(404).json({ success: false, message: "No returns found for this product" });
    }

    res.json({ success: true, returns });
  } catch (err) {
    console.error("Error fetching returns by product:", err);
    res.status(500).json({ success: false, message: "Failed to get returns by product" });
  }
};
