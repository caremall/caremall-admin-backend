...
export const markReturnComplete = async (req, res) => {
    try {
        ...
        if (returnDoc.status !== 'approved') {
            return res.status(400).json({ success: false, message: 'Return must be approved before completion' });
        }
        ...
        res.status(500).json({ success: false, message: 'Failed to complete return' });
    }
};
...
export const updatePickupStatus = async (req, res) => {
    try {
        ...
        if (pickupScheduled !== undefined) returnDoc.pickupScheduled = pickupScheduled;
