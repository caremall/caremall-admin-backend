  ...
  try {
    ...
    for (let variant of variants) {
      if (variant.SKU) {
        ...
        const existsInProduct = await Product.findOne({
      ...
      if (variant.barcode) {
        ...
        if (exists || existsInProduct)
          return res
            .status(400)
            .json({ message: `Barcode '${variant.barcode}' already exists` });
      }
    }
  ...
  try {
    ...
    if (req.body.SKU && req.body.SKU !== variant.SKU) {
      ...
      if (exists || existsInProduct)
        return res
          .status(400)
          .json({ message: `SKU '${req.body.SKU}' already exists` });
    }
    if (req.body.barcode && req.body.barcode !== variant.barcode) {
      ...
      if (exists || existsInProduct)
        return res
          .status(400)
          .json({ message: `Barcode '${req.body.barcode}' already exists` });
    }
